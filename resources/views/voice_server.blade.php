...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // 🟢 YENİ: Aynı origin + HTTPS, websocket zorunlu
    const socket = io({
        query: { session_id: sessionId },
        transports: ["websocket"],   // polling'e düşmesin
        path: "/socket.io"           // default ama net olsun
    });

    socket.emit("joinSession", { session_id: sessionId });

    socket.on("server_hello", (data) => {
        console.log("SERVER_HELLO (voice):", data);
    });
        ...
        intervalId = setInterval(() => {
            recorderStream.exportWAV(async (blob) => {
                ...
                try {
                    ...
                    // 🔴 ESKİ: IP + HTTP
                    // const res = await fetch("http://167.99.253.53:5001/stt", {

                    // 🟢 YENİ: Aynı origin, HTTPS (relative URL)
    ...
    stopBtn.addEventListener("click", () => {
        ...
        recorderFull.exportWAV(async (blob) => {
            ...
            try {
                // 🔴 ESKİ: IP + HTTP
                // const res = await fetch("http://167.99.253.53:5001/stt_translate_tts", {

                // 🟢 YENİ: Aynı origin
                const res = await fetch("/stt_translate_tts", {
                    method: "POST",
                    body: form
                });
